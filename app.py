...
app.config['SECRET_KEY'] = 'final_project_g3'
...
def index():
    if request.method == 'POST':
        ...
        if stock:
            session['stock'] = stock  # Store user input in session
        else:
            session['stock'] = '^NSEI'  # Default stock if none is entered

    # Get stock value from session (retain after page refresh)
    stock = session.get('stock', '^NSEI')
    ...
    ax1.plot(ema20, 'g', label='EMA 20', linewidth = 1)
    ax1.plot(ema50, 'r', label='EMA 50', linewidth = 1)
    ...
    ax2.plot(ema100, 'g', label='EMA 100', linewidth = 1)
    ax2.plot(ema200, 'r', label='EMA 200', linewidth = 1)
    ...
    return render_template('index.html', 
                           ...
                           stock=stock)  # Pass stock value to template
